...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        - seed: If not None, then pass this random seed to the dropout layers. This
          will make the dropout layers deteriminstic（确定性的） so we can gradient check the
          model.
        """
        ...
        total_dims = deque(hidden_dims)
        total_dims.appendleft(input_dim)
        total_dims.append(num_classes)
        start_dim = total_dims.popleft()

        i = 1
        while len(total_dims) > 0:
            end_dim = total_dims.popleft()
            w, b = 'W{}'.format(i), 'b{}'.format(i)
            self.params[w] = np.random.normal(loc=0, scale=weight_scale,
                                              size=(start_dim, end_dim))
            self.params[b] = np.zeros((end_dim,))
            if (self.normalization == 'batchnorm' or self.normalization == 'layernorm') and i < self.num_layers:
                ...
                self.params[gamma] = np.ones((end_dim,))
                self.params[beta] = np.zeros((end_dim,))
            start_dim = end_dim
            i += 1

        # 参考答案，如果没有隐藏层，会报 IndexError。
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            ...
            if i == self.num_layers - 1:
                ...
                if self.normalization is not None:
                    gamma, beta = 'gamma{}'.format(i + 1), 'beta{}'.format(i + 1)
                    out, cache = affine_norm_relu_forward(out, self.params[w], self.params[b], self.params[gamma],
                                                          self.params[beta], self.bn_params[i], self.normalization)
                else:
        ...
        reg_w_loss = 0
        for i in range(self.num_layers):
            reg_w_loss += np.sum(self.params['W{}'.format(i + 1)] ** 2)
        loss += 0.5 * self.reg * reg_w_loss
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            if i == self.num_layers:
                ...
                if self.normalization is not None:
                    gamma, beta = 'gamma{}'.format(i), 'beta{}'.format(i)
                    dx, dgamma, dbeta, dw, db = affine_norm_relu_backward(dx, cache, self.normalization)
                    grads[gamma] = dgamma
                    grads[beta] = dbeta
                else:
        ...
        # print('grad keys', grads.keys(), self.num_layers)
        # print('param keys', self.params.keys(), self.num_layers)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
